...
This repository is a Cursor plugin marketplace — the root `.cursor-plugin/marketplace.json` lists every plugin below. Publish it via [cursor.com/marketplace/publish](https://cursor.com/marketplace/publish) (requires a permissive license), or install a single plugin locally:

```bash
git clone <this-repo>
cp -r agents-store-cursor-plugins/<plugin-name> ~/.cursor/plugins/local/<plugin-name>
```
